                      ...
                      <div className="space-y-2 flex-grow z-10">
                        {creativeRanking?.worst?.length ? creativeRanking.worst.map((ad:any, i:number) => (
                           <div key={i} onClick={() => setSelectedAdModal({...ad, rank: 'worst'})} className="flex gap-3 bg-background/80 p-2.5 rounded-lg border border-destructive/20 items-center transition-all hover:bg-background hover:scale-[1.02] hover:shadow-[0_8px_20px_rgba(0,0,0,0.12)] cursor-pointer shadow-sm relative group/ad">
                              ...
                              <div className="flex-1 min-w-0">
                                  <p className="text-xs font-bold truncate text-foreground/90 w-full" title={ad.ad_name}>{ad.ad_name}</p>
                                  <div className="flex justify-between items-center mt-1.5 gap-2"><span className="text-[10px] font-mono text-destructive/80 font-semibold truncate shrink-0">Lixo {formatCurrency(ad.spend)}</span><Badge variant="destructive" className="bg-destructive/80 text-[9px] px-1.5 py-0 shadow-sm shrink-0 truncate max-w-[85px]" title={`${ad.results} ${ad.results === 1 ? ad.resultType : ad.resultType + 's'}`}>{ad.results} {ad.results === 1 ? ad.resultType : ad.resultType + 's'}</Badge></div>
                              </div>
                           </div>
                    ...
                    <div className="md:w-7/12 flex flex-col bg-background relative overflow-hidden">
                        ...
                        <div className="p-6 flex-1 overflow-y-auto space-y-6 scrollbar-thin">
                           ...
                           <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              <div className="p-3 bg-secondary/10 rounded-lg border border-border/50 text-center"><p className="text-[10px] uppercase text-muted-foreground font-bold mb-1">Volume</p><p className="text-sm font-mono font-bold text-primary truncate" title={`${ad.results} ${ad.results === 1 ? ad.resultType : ad.resultType + 's'}`}>{ad.results} <span className="text-[9px] uppercase opacity-80">{ad.results === 1 ? ad.resultType : ad.resultType + 's'}</span></p></div>
