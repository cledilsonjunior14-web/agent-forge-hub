...
import { LayoutGrid, List, ArrowUpRight, ArrowDownRight, Video } from 'lucide-react';
...
export default function CreativesPage() {
  ...
  // Aqui poderia filtrar por selectedCampaigns se houvesse relation nos mocks. Para UX, usaremos data full.
    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      ...
      {viewMode === 'grid' ? (
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
            {creatives.map((c, i) => (
               <div key={i} className="card overflow-hidden hover:-translate-y-1 transition-transform group">
                  ...
                  <div className="p-4">
                     <h4 className="font-bold text-sm text-text-primary mb-1 truncate">{c.name}</h4>
                     <p className="text-[10px] text-text-secondary uppercase tracking-widest mb-4">Campanha: Toro Burger</p>
                     
                     <div className="space-y-2 mb-4">
                        <div className="flex justify-between items-center text-xs">
                           <span className="text-text-muted font-medium">CPA / Lixo</span>
                  ...
                  <tbody className="divide-y divide-border-subtle bg-bg-base">
                     {creatives.map((c, i) => (
                        <tr key={i} className="hover:bg-bg-elevated transition-colors">
                           ...
                           <td className="px-4 py-2 font-bold text-text-primary">{c.name}</td>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="card bg-[#00E096]/[0.02] border-[#00E096]/20 overflow-hidden shadow-[0_0_50px_rgba(0,224,150,0.03)]">
               ...
               <div className="p-5 space-y-3">
                  {promessas.map((c, i) => (
                     <div key={i} className="flex items-center justify-between p-3 bg-bg-surface border border-border-default rounded-lg hover:border-[#00E096]/30 transition-colors group cursor-pointer">
                        <div className="flex items-center gap-4">
                           <div className="w-12 h-12 rounded bg-bg-overlay flex items-center justify-center text-text-muted text-[8px] font-bold group-hover:scale-105 transition-transform">TH</div>
                           <div>
            ...
            <div className="card bg-[#FF4D4D]/[0.02] border-[#FF4D4D]/20 overflow-hidden shadow-[0_0_50px_rgba(255,77,77,0.03)]">
               ...
               <div className="p-5 space-y-3">
                  {sangrias.map((c, i) => (
                     <div key={i} className="flex items-center justify-between p-3 bg-bg-surface border border-border-default rounded-lg hover:border-[#FF4D4D]/30 transition-colors group cursor-pointer">
                        <div className="flex items-center gap-4">
                           <div className="w-12 h-12 rounded bg-[#FF4D4D]/5 border border-[#FF4D4D]/20 flex items-center justify-center text-[#FF4D4D]/50 text-[8px] font-bold group-hover:scale-105 transition-transform">TH</div>
                           <div>
                              <p className="text-sm font-bold text-text-primary">{c.name}</p>
                        ...
                        <div className="flex flex-col items-end">
                           ...
                           <span className="text-[9px] text-[#FF4D4D]/60 mt-1 uppercase tracking-wider font-bold">Bloqueio Urgent</span>
                        </div>
                     </div>
                  ))}
               </div>
